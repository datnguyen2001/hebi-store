@if(count($product_phone) > 0)
    @php
        $category_product = \App\Models\CategoryModel::where('display', 1)->where('parent_id', 0)->where('type', 1)->limit(5)->get();
    @endphp
    <div class="phone">
        <div class="title-block">
            ...
            <div class="list-cat">
                @foreach($category_product as $item)
                    <a href="{{url('danh-muc/dien-thoai/'.$item->slug)}}" class="item-cat"><span>{{$item->name}}</span></a>
                @endforeach
                <a href="{{url('danh-muc/dien-thoai')}}" class="item-cat"><span>Xem tất cả sản phẩm</span></a>
            </div>
        </div>
