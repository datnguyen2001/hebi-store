@if(count($product_tablet) > 0)
    @php
        $category_product = \App\Models\CategoryModel::where('display', 1)->where('parent_id', 0)->where('type', 2)->limit(5)->get();
    @endphp
    <div class="tablet">
        <div class="title-block">
            ...
            <div class="list-cat">
                @foreach($category_product as $item)
                    <a href="{{url('danh-muc/may-tinh-bang/'.$item->slug)}}"
                       class="item-cat"><span>{{$item->name}}</span></a>
                @endforeach
                <a href="{{url('danh-muc/may-tinh-bang')}}" class="item-cat"><span>Xem tất cả sản phẩm</span></a>
            </div>
        </div>
        <div class="list-products row">
            @foreach($product_tablet as $value)
