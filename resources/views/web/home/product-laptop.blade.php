@if(count($product_laptop) > 0)
    @php
        $category_product = \App\Models\CategoryModel::where('display', 1)->where('parent_id', 0)->where('type', 3)->limit(5)->get();
    @endphp
    <div class="watch">
        <div class="title-block">
            ...
            <div class="list-cat">
                @foreach($category_product as $item)
                    <a href="{{url('danh-muc/laptop/'.$item->slug)}}" class="item-cat"><span>{{$item->name}}</span></a>
                @endforeach
                <a href="{{url('danh-muc/laptop')}}" class="item-cat"><span>Xem tất cả sản phẩm</span></a>
            </div>
        </div>
